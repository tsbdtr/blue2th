...
- When a user describes a new feature, **before** filling `tdd/feature.md` and before running `/tdd`:
  ...
  5. Only once the scenarios and impact are clear: fill `tdd/feature.md` — the **Layers touched** checkboxes, and the **Manual verification** section with what no test can check *and how to trigger it*.
