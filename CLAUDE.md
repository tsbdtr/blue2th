...
- When a user describes a new feature, **before** filling `tdd/feature.md` and before running `/tdd`:
  ...
  5. Only once the scenarios and impact are clear: fill `tdd/feature.md` — the **Change Type** (the Conventional Commits type the branch and the pull request will carry), the **Tracking Issue** (`#N` when the work is already filed, `none` otherwise), the **Layers touched** checkboxes, the **Guards** (for each *never / only / exactly / empty* criterion, the near-miss input only that guard rejects), the **Captured fixtures** (real data a spike or a measurement already produced, with its provenance), and the **Manual verification** section with what no test can check *and how to trigger it*.
